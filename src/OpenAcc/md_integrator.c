																		...
																		su3_soa *local_staples, tamat_soa *tipdot,thmat_soa *tmomenta, int omelyan_index)
{
	...
	// this function should have different in and out for the gauge conf
	// embarrassingly parallel
															 ...
															 const int max_cg)
{
	...
	for(md=1; md<md_parameters.no_md; md++){
		...
		if(md_parameters.extrapolateInvsForce){
			...
			calc_new_trialsol_for_inversion_in_force(totalMdShifts,tferm_shiftmulti_acc,
																							 nMdInversionPerformed); // nMdInversionPerformed even - trial solution in the first half of the vector
		...
		if(md_parameters.extrapolateInvsForce){
			printf("ERROR, not implemented correctly! %s : %d",__FILE__,__LINE__); exit(1);
			calc_new_trialsol_for_inversion_in_force(totalMdShifts,tferm_shiftmulti_acc,
																							 nMdInversionPerformed); // nMdInversionPerformed odd - trial solution in the second half of the vector
			invOuts = &tferm_shiftmulti_acc[totalMdShifts];
		}        
	...
	if(md_parameters.extrapolateInvsForce){ 
		printf("ERROR, not implemented correctly! %s : %d",__FILE__,__LINE__); exit(1);
		calc_new_trialsol_for_inversion_in_force(totalMdShifts,tferm_shiftmulti_acc,
																						 nMdInversionPerformed); // nMdInversionPerformed even - trial sol in the first half of the vector
		invOuts = tferm_shiftmulti_acc;
	}        
